...
saveAs("Tiff", currentDir+"/MAX_C4_grey_Thresh.tif");
...
function calculatePunctaCytoplasm(i) {
	...
	saveAs("Results", currentDir+"/TDPpuncta_"+i+".csv");
	close("Results");
}
...
open("MAX_C4_grey_Thresh.tif");
open("ROIs.zip");
for (i=0; i<cellCount; i++) {
	flattenROIs(i+1);
	}
saveAs("Tiff", currentDir+"/Flat of MAX_C4_grey_Thresh.tif");
close("*");
selectWindow("ROI Manager");
run("Close");
...
selectWindow("Mask of MAX_C4_grey_Thresh.tif");
run("Invert");
roiManager("Select", findIndexByName("cytoplasm1"));
run("Flatten");
saveAs("Tiff", currentDir+"/Mask of MAX_C4_grey.tif");
close("Results");
close("*");
selectWindow("ROI Manager");
run("Close");